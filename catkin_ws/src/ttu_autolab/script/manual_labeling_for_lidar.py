...
# -*- coding: utf-8 -*-
'''
Script to manual labeling out the contour of car and human for lidar image.
'''
import cv2
import numpy as np
import argparse
...
#            break
        ...
        cv2.imshow(w_name, img_render)
        cv2.setMouseCallback(w_name, mouse_click_callback)
    anno_path = w_name.replace('lidar_blank', 'annotation_rgb')
...
if __name__ == '__main__':
    ...
    img_rgb_path = args.image.replace('lidar_blank', 'lidar_rgb')

    image_rgb = cv2.imread(img_rgb_path)
    image_blank = cv2.imread(args.image)  # read the image

    if not (args.geometry is None):
        width, height = [int(v) for v in args.geometry.split('x')]
        print('Scaling down to %dx%d' % (width, height))

        img_blank_scaled = cv2.resize(image_blank, (width, height),
                                      interpolation=cv2.INTER_CUBIC)
        image_blank = img_blank_scaled
