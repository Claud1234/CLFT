...
{
    sub_lidar = nh.subscribe("/lidar_front/velodyne_points", 1000, &MapGenerationNode::lidarCallback, this);

	ros::param::set("~image_transport", "compressed");
	sub_camera = it.subscribe("/front_camera/image_color_rect", 1000, &MapGenerationNode::cameraCallback, this);
...
void MapGenerationNode::lidarCallback(const sensor_msgs::PointCloud2::ConstPtr& lidar)
{
	...
	char s[1000];
	std::string pcd_path = ros::package::getPath("ttu_autolab") + "/output/pcd/";
	//const char * pcd_path_1 = pcd_path.c_str();
	sprintf(s, (pcd_path + "%06lld.pcd").c_str(), lidar_index);
	//sprintf(s, "/home/claude/Dev/TalTech_DriverlessProject/catkin_ws/src/ttu_autolab/output/pcd/%06lld.pcd", lidar_index);
	++lidar_index;
...
void MapGenerationNode::cameraCallback(const sensor_msgs::ImageConstPtr& camera)
{
	...
	char s[1000];
	std::string png_path = ros::package::getPath("ttu_autolab") + "/output/png/";
	sprintf(s, (png_path + "%06lld.png").c_str(), lidar_index-1);
	//sprintf(s, "/home/claude/Dev/TalTech_DriverlessProject/catkin_ws/src/ttu_autolab/output/png/%06lld.png", lidar_index-1);
