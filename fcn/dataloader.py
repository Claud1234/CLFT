...
class SemiDataset():
    '''
    No waymo dataset process, no process to annotations.
    '''
    def __init__(self, rootpath, split, augment):
        np.random.seed(789)
        self.rootpath = rootpath
        self.split = split
        self.augment = augment
    ...
    def __getitem__(self, idx):
        ...
        top_crop_class = TopCropSemi(cam_path, lidar_path)
        # Apply top crop for raw data to crop the 1/2 top of the images
        top_crop_rgb, top_crop_points_set,\
            top_crop_camera_coord = top_crop_class.top_crop()

        augment_class = AugmentShuffleSemi(top_crop_rgb, top_crop_points_set,
                                           top_crop_camera_coord)
        if self.augment is not None:
            if self.augment_shuffle is True:
                aug_list = ['random_crop', 'random_rotate', 'colour_jitter',
                ...
                for i in range(len(aug_list)):
                    augment_proc = getattr(augment_class, aug_list[i])
                    rgb, X, Y, Z = augment_proc()

            else:  # self.augment_shuffle is False
                rgb, X, Y, Z = augment_class.random_crop()
                rgb, X, Y, Z = augment_class.random_rotate()
                rgb, X, Y, Z = augment_class.colour_jitter()
                rgb, X, Y, Z = augment_class.random_horizontal_flip()
                rgb, X, Y, Z = augment_class.random_vertical_flip()
